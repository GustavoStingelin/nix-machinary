...
| co-existing | `existing-focus.txt`, `real-git.txt` | `internal/app` existing-branch real-Git tests |
| co-new | `new-worktree.txt`, `new-collision.txt` | `internal/app` new-branch real-Git tests |
| pr | `pull-request.txt` covers create, reuse, malformed metadata, recovery, and legacy prefix non-adoption | `internal/app` and `internal/github` PR real-Git tests |
